...
import (
	...
	"wx.com/redis/internal"
	"wx.com/redis/internal/consistenthash"
	"wx.com/redis/internal/hashtag"
...
func (ring *Ring) GetShards() map[string]*ringShard {
	return ring.shards
}
...
func (ring *Ring) getClient(key string) (*Client, error) {
	...
	ring.mx.RUnlock()
	glog.Info(name, "-", cl.opt.Addr, " key:", key)
	return cl, nil
}
	...
	for _ = range ticker.C {
		...
		for _, shard := range ring.shards {
			...
			if shard.Vote(err == nil || err == pool.ErrPoolTimeout) {
				...
				glog.Error("redis 节点宕机:", shard, " ", shard.down)
				rebalance = true
			}
		}
